...
import { MSN_API_URL } from "../../config/config.js";

export class MSNProvider {
  static async get(endpoint, config) {
    try {
      const response = await axios.get(MSN_API_URL + endpoint, {
        headers: {
  ...
  static async post(payload, endpoint, config) {
    try {
      const response = await axios.post(MSN_API_URL + endpoint, payload, {
        headers: {
